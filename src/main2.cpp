  ...
  if(PathManager::GetInstance()->NumPaths() > 0) {
    ...
    while (true) {
      auto command = controller.Update(TankOdometry::GetInstance()->GetPose(), pros::millis() / 1000.0);
  		...
  		TankOdometry::GetInstance()->Update(frontLeft.getEncoder()->get(), frontRight.getEncoder()->get(), 10); //Using pure encoder based odometry as gyro odometry seems to be having issues
