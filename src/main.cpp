...
void opcontrol() {
  ...
  backRight.setReversed(true);
	...
	TankOdometry::GetInstance()->Initialize(encoderConfig, encoderConfig, Pose(Vector2d(paths[1].getFirstWaypoint().position.getY(),-paths[1].getFirstWaypoint().position.getX()), Rotation2Dd(0)));
  ...
  if(PathManager::GetInstance()->NumPaths() > 0) {
    ...
    while (true) {
      auto command = controller.Update(TankOdometry::GetInstance()->GetPose(), pros::millis() / 1000.0);
  		...
  		TankOdometry::GetInstance()->Update(frontLeft.getEncoder()->get(), frontRight.getEncoder()->get(), 10.3); //Using pure encoder based odometry as gyro odometry seems to be having issues
